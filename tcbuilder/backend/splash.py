...
def create_splash_initramfs(work_dir, image, sysroot_dir, src_ostree_archive_dir):
    try:
        ...
        initramfs = Gio.File.new_for_path(os.path.join(work_dir, "usr/lib/modules",
                                                    kernel_version, "initramfs.img")).create(
                                                    Gio.FileCreateFlags.NONE, None)
        
        initramfs.splice(sub_path.read(None), Gio.OutputStreamSpliceFlags.CLOSE_SOURCE, None)
        
        initramfs.splice(Gio.File.new_for_path(os.path.join(work_dir,
                                        splash_initramfs)).read(None),
                                        Gio.OutputStreamSpliceFlags.CLOSE_SOURCE | Gio.OutputStreamSpliceFlags.
                                        CLOSE_TARGET, None)
        
        os.remove(os.path.join(work_dir, splash_initramfs))
    except Exception as ex:
        raise TorizonCoreBuilderError("issue occurred during creating initramfs. Contact Developer") \
            from ex
